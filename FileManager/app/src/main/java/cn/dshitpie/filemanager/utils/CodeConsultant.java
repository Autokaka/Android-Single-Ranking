package cn.dshitpie.filemanager.utils;

public class CodeConsultant {
    //Activity
    public static final int MAIN_ACTIVITY = 0;
    public static final int ADD_ITEM_ACTIVITY = 1;
    public static final int MENU_ACTIVITY = 2;
    public static final int MENU_ACTIVITY_DELETE = 3;
    public static final int MENU_ACTIVITY_RENAME = 4;
    public static final int RENAME_ACTIVITY = 5;
    //FileManager
    public static final int FILE_NOT_EXISTS = 2;
    public static final int FILE_ALREADY_EXISTS = 1;
    public static final int OPERATE_SUCCESS = 0;
    public static final int OPERATE_FAIL = -1;
    //AddItemActivity
    public static final int TYPE_FILE = 0;
    public static final int TYPE_DIRECTORY = 1;
}
